...
# (Tidak ada perubahan di bagian ini, tetap sama seperti sebelumnya)
# =============================================================
...
def make_logistic_obj_lgb(vol_vec, hess_min=1e-6):
...
def lgbpp_py(data, vol, params, loss="poisson", F_prime=1, **kwargs):
    ...
    elif loss == "logistic": objective_func = make_logistic_obj_lgb(vol)
    ...
    elif loss == "logistic": eval_func = make_logistic_metric_lgb(vol, F_prime)
    else: raise ValueError(f"Loss '{loss}' tidak dikenali.")
...
def tune_lgbpp(X_df, y_series, vol_series, loss = "poisson", F_prime=1.0, n_trials=100, constrain_events=False, constraint_strength=1.0):
    ...
    def objective_optuna(trial):
        ...
        if loss == "poisson":
            ...
            right_val = num_events_val  # consistent

        elif loss == "logistic":
            delta_dummy = 1.0 / vol_dummy
            delta_event = 1.0 / vol_series.values[(y_series.values > 0)]

            left_logistic_log_likelihood = np.sum(
                y_pred_event - np.log(delta_event + np.exp(y_pred_event))
            )

            right_logistic_log_likelihood = np.sum(
                np.log(delta_dummy) - np.log(delta_dummy + np.exp(y_pred_dummy))
            )

            log_likelihood = left_logistic_log_likelihood + right_logistic_log_likelihood

            left_val = left_logistic_log_likelihood
            right_val = right_logistic_log_likelihood  # ✅ FIXED

            # still needed for constraint (same as Poisson, as you requested)
            num_events_val = np.sum(np.exp(y_pred_dummy) * vol_dummy)
        ...
        trial.set_user_attr("right", right_val)  # ✅ now correct for both cases
    ...
    model_training_time = end_time_model - start_time_model

    # ================================
    # SAVE TIME TO TXT
    # ================================
    txt_path = os.path.join(results_dir, "final_model_time.txt")

    with open(txt_path, "w") as f:
        f.write(f"Final Model Training Time (seconds): {model_training_time:.6f}\n")
        f.write(f"Final Model Training Time (minutes): {model_training_time/60:.6f}\n")

    print(f"Waktu training model final: {model_training_time:.2f} detik")
    print(f"Disimpan di: {txt_path}")
