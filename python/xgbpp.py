...
def tune_xgbpp(X_df, y_series, vol_series, loss, F_prime=1.0, n_trials=100, constrain_events=False, constraint_strength=1.0):
    ...
    def objective_optuna(trial):
        ...
        if loss in ["poisson", "weighted_poisson"]:
            ...
            right_val = num_events_val  # ✅ consistent with likelihood
            ...
            # LEFT (event likelihood part)
            left_val = np.sum(
                y_pred_event - np.log(delta_event + rho_event)
            )

            # RIGHT (true likelihood part)
            right_val = np.sum(
                np.log(delta_dummy) - np.log(delta_dummy + rho_dummy)
            )

            log_likelihood = left_val + right_val

            # 🔴 SAME CONSTRAINT STYLE AS POISSON
            num_events_val = np.sum(np.exp(y_pred_dummy) * vol_dummy)
        ...
        trial.set_user_attr("right", right_val)  # ✅ FIXED
