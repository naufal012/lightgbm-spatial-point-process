...
# 5. QUADRATURE SCHEME
# ================================
Q_lin <- quadscheme.logi.linnet(kcl_point, dummytype = 'binomial')
...
train_lgbpp_fixed <- function(X, y, vol, loss = 'logistic', F_prime) {
...
train_xgbpp_fixed <- function(X, y, vol, loss = 'logistic', F_prime) {
...
run_models_LPP <- function(final_data, L, kcl_point, output_dir = "results_ml_logi", type = 'squared', use_tuning = FALSE, n_trials = 20) {
  ...
  for (model_type in model_list) {
    ...
    if (model_type == "xgb") {
      ...
      if (use_tuning) {
        ...
        model <- train_xgbpp_fixed(X, y, vol, "logistic", F_prime)
      }
      ...
      if (use_tuning) {
        ...
        model <- train_lgbpp_fixed(X, y, vol, "logistic", F_prime)
      }
    ...
    # ================================
    delta <- 1/vol
    left_logistic_log_likelihood <- sum((log(exp(preds[y == 1])/(delta[y == 1] + exp(preds[y == 1])))))
    right_logistic_log_likelihood <- sum((log(delta[y == -1]/(delta[y == -1] + exp(preds[y == -1])))))
    ...
    if (model_type == "xgb") {
      ...
      write.xlsx(sim_data, "results_dataframe_xgb_logi.xlsx", rowNames = FALSE)
    } else {
      fi <- plot_feature_importance_lgb(model, features_cols)
      write.xlsx(sim_data, "results_dataframe_lgb_logi.xlsx", rowNames = FALSE)
    }
    ...
    cat(
      paste(
        ...
        "\nLogistic Log-Likelihood:", round(logistic_log_likelihood, 4),
