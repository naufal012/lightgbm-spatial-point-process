...
run_analysis <- function(model_type, loss_type, sim_data, sim_intensity, sim_points, 
  ...
  plot_feature_importance_xgb <- function(model, features_cols) {
    # Get feature importance from the Python model
    py_feature_importances <- model$get_score(importance_type = 'gain')
    
    # Convert to R data frame
    importance_df <- data.frame(
      feature = names(py_feature_importances),
      importance = as.numeric(py_feature_importances)
    )
    importance_df$importance_pct <- importance_df$importance / sum(importance_df$importance)
    ...
    return(list(plot = p, data = importance_df))
  }
  
  plot_feature_importance_lgb <- function(model) {
    # Get gain importance from Python LightGBM model
    gain_imp <- py_to_r(model$feature_importance(importance_type = "gain"))
    
    # Get feature names
    feat_names <- py_to_r(model$feature_name())
    
    # Build data frame
    importance_df <- data.frame(
      feature = feat_names,
      importance = as.numeric(gain_imp),
      stringsAsFactors = FALSE
    )
    importance_df$importance_pct <- importance_df$importance / sum(importance_df$importance) * 100
  ...
  fi_result <- if (model_type == "xgb") {
    plot_feature_importance_xgb(final_model)
  } else {
    plot_feature_importance_lgb(final_model)
  }
  
  write.csv(
    fi_result$data,
    file.path(output_folder, "feature_ranks.csv"),
    row.names = FALSE
  )
  
  if (!is.null(fi_result$plot)) {
    ggsave(filename = file.path(viz_folder, "feature_importance_plot.png"), plot = fi_result$plot, width = 8, height = 6)
  }
